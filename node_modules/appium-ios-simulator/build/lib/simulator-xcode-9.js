...
const startupLock = new async_lock_1.default();
const preferencesPlistGuard = new async_lock_1.default();
...
function compileSimulatorPreferences(opts = {}) {
    ...
    switch (lodash_1.default.lowerCase(pasteboardAutomaticSync)) {
        ...
        case 'system':
            ...
            break;
        default:
            logger_1.default.info(`['on', 'off' or 'system'] are available as the pasteboard automatic sync option. Defaulting to 'off'`);
            commonPreferences.PasteboardAutomaticSync = false;
    }
    return [devicePreferences, commonPreferences];
}
class SimulatorXcode9 extends simulator_xcode_8_1.default {
    ...
    async run(opts = {}) {
        ...
        const shouldWaitForBoot = await startupLock.acquire(this.uiClientBundleId, async () => {
            ...
            if (opts.isHeadless) {
                if (isServerRunning && !uiClientPid) {
                    logger_1.default.info(`Simulator with UDID '${this.udid}' is already booted in headless mode.`);
                    return false;
                }
                if (await this.killUIClient({ pid: uiClientPid })) {
                    logger_1.default.info(`Detected the Simulator UI client was running and killed it. Verifying the current Simulator state`);
                }
                try {
                ...
                logger_1.default.info(`Booting Simulator with UDID '${this.udid}' in headless mode. ` +
                ...
                if (isServerRunning) {
                    logger_1.default.info(`Simulator '${this.udid}' is booted while its UI is not visible. ` +
        ...
        (async () => {
            try {
                ...
                logger_1.default.info(`Cannot disable Simulator keyboard introduction. Original error: ${e.message}`);
            }
        })();
    }
    /**
    ...
    async startUIClient(opts = {}) {
        ...
        const simulatorApp = path_1.default.resolve(await (0, appium_xcode_1.getPath)(), 'Applications', utils_js_1.SIMULATOR_APP_NAME);
        const args = ['-Fn', simulatorApp];
        logger_1.default.info(`Starting Simulator UI: ${support_1.util.quote(['open', ...args])}`);
        try {
     ...
     */
    async boot() {
        const bootEventsEmitter = new events_1.EventEmitter();
    ...
    verifyDevicePreferences(prefs = {}) {
        ...
        if (!lodash_1.default.isUndefined(prefs.SimulatorWindowLastScale)) {
            if (!lodash_1.default.isNumber(prefs.SimulatorWindowLastScale) || prefs.SimulatorWindowLastScale <= 0) {
                logger_1.default.errorAndThrow(`SimulatorWindowLastScale is expected to be a positive float value. ` +
        ...
        if (!lodash_1.default.isUndefined(prefs.SimulatorWindowCenter)) {
            ...
            if (!lodash_1.default.isString(prefs.SimulatorWindowCenter) || !verificationPattern.test(prefs.SimulatorWindowCenter)) {
                logger_1.default.errorAndThrow(`SimulatorWindowCenter is expected to match "{floatXPosition,floatYPosition}" format (without spaces). ` +
        ...
        if (!lodash_1.default.isUndefined(prefs.SimulatorWindowOrientation)) {
            ...
            if (!prefs.SimulatorWindowOrientation || !acceptableValues.includes(prefs.SimulatorWindowOrientation)) {
                logger_1.default.errorAndThrow(`SimulatorWindowOrientation is expected to be one of ${acceptableValues}. ` +
        ...
        if (!lodash_1.default.isUndefined(prefs.SimulatorWindowRotationAngle)) {
            if (!lodash_1.default.isNumber(prefs.SimulatorWindowRotationAngle)) {
                logger_1.default.errorAndThrow(`SimulatorWindowRotationAngle is expected to be a valid number. ` +
    ...
    async updatePreferences(devicePrefs = {}, commonPrefs = {}) {
        if (!lodash_1.default.isEmpty(devicePrefs)) {
            logger_1.default.debug(`Setting preferences of ${this.udid} Simulator to ${JSON.stringify(devicePrefs)}`);
        }
        if (!lodash_1.default.isEmpty(commonPrefs)) {
            logger_1.default.debug(`Setting common Simulator preferences to ${JSON.stringify(commonPrefs)}`);
        }
        const homeFolderPath = process.env.HOME;
        if (!homeFolderPath) {
            logger_1.default.warn(`Cannot get the path to HOME folder from the process environment. ` +
        ...
        return await preferencesPlistGuard.acquire(SimulatorXcode9.name, async () => {
            ...
            try {
                ...
                logger_1.default.debug(`Updated ${this.udid} Simulator preferences at '${plistPath}' with ` +
                    JSON.stringify(prefsToUpdate));
                return true;
            }
            catch (e) {
                logger_1.default.warn(`Cannot update ${this.udid} Simulator preferences at '${plistPath}'. ` +
    ...
    async isBiometricEnrolled() {
        ...
        logger_1.default.info(`Current biometric enrolled state for ${this.udid} Simulator: ${match[1]}`);
        return match[1] === '1';
    }
    /**
     *
    ...
    async enrollBiometric(isEnabled = true) {
        logger_1.default.debug(`Setting biometric enrolled state for ${this.udid} Simulator to '${isEnabled ? 'enabled' : 'disabled'}'`);
    ...
    async sendBiometricMatch(shouldMatch = true, biometricName = 'touchId') {
        const domainComponent = (0, utils_js_1.toBiometricDomainComponent)(biometricName);
        const domain = `com.apple.BiometricKit_Sim.${domainComponent}.${shouldMatch ? '' : 'no'}match`;
        await this.simctl.spawnProcess([
            'notifyutil',
            '-p', domain
        ]);
        logger_1.default.info(`Sent notification ${domain} to ${shouldMatch ? 'match' : 'not match'} ${biometricName} biometric ` +
    ...
    async getLaunchDaemonsRoot() {
        const devRoot = await (0, utils_js_1.getDeveloperRoot)();
...
//# sourceMappingURL=simulator-xcode-9.js.map